...
- **Retry · partial** — attempt 1 lands 4 of 7 words. The words that did not come through go **inactive**, the card warms to golden brown, fill and border, and the **mic goes to sleep and says why** ("You missed a few words") while the coach bubble steps out. Then the **speaker takes over**, replaying the phrase karaoke-style with the bubble carrying "Listen once, then read it again". Only when it finishes does the **mic wake up** with a ring and "Your turn". The second read lands and "Nice" arrives as a top notification.
...
The right-side checkbox steps the short-read state back to **border only**, for comparing against the default where the whole card warms.
